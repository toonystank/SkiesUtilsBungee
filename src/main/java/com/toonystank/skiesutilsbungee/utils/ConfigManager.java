...
package com.toonystank.skiesutilsbungee.utils;
...
public class ConfigManager {
    ...
    private void process(Plugin plugin, String fileName, String path, boolean force, boolean copy) throws IOException {
        ...
        if (!file.exists()) {
            ...
            if (copy) try {
                copy(force, Paths.get(path + File.separator + fileName));
            } catch (IllegalArgumentException e) {
                e.printStackTrace();
            }
            else {
    ...
    public void copy( boolean force) {
        if (!file.exists()) {
            copy(force, plugin.getDataFolder().toPath());
        }
    }
    /**
     ...
     */
    public void copy( boolean force, Path path) {
